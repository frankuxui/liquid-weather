...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 px-4 pt-4">
      <nav className="glass mx-auto flex max-w-6xl items-center justify-between rounded-full px-5 py-2.5 backdrop-blur-lg">
            ...
            {LINKS.map((link) => {
              ...
              return (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className={cn("rounded-full px-4 py-2 text-sm font-medium transition-colors", active ? "bg-white/12 text-foreground" : "text-foreground/70 hover:bg-white/8 hover:text-foreground")}
                  >
                    {link.label}
                  </Link>
                </li>
