...
export const WIDGETS: WidgetDef[] = [
  {
    ...
    render: (w) => (
      <WidgetShell title="Sensación térmica" icon={<ThermometerSun size={16} />}>
        <BigValue
          value={`${Math.round(w.current.apparentTemperature)}°`}
          sub={`Real ${Math.round(w.current.temperature)}°`}
        />
      </WidgetShell>
    ),
  },
  ...
  },
  {
    ...
    render: (w) => (
      <WidgetShell
        title="Viento"
        icon={<Wind size={16} />}
        accent="hsl(160 84% 55%)"
        footer={`Rachas de ${Math.round(w.current.windGusts)} km/h`}
      >
        <BigValue
          value={`${Math.round(w.current.windSpeed)}`}
          unit="km/h"
          sub={`Dirección ${windDirectionLabel(w.current.windDirection)}`}
        />
      </WidgetShell>
    ),
  },
  ...
  },
  {
    ...
    render: (w) => (
      <WidgetShell title="Presión" icon={<Gauge size={16} />} accent="hsl(48 96% 60%)">
        <BigValue
          value={`${Math.round(w.current.pressure)}`}
          unit="hPa"
          sub={
            w.current.pressure > 1013
              ? "Presión alta"
              : w.current.pressure < 1009
                ? "Presión baja"
                : "Presión normal"
          }
        />
    ...
    render: (w) => (
      <WidgetShell title="Visibilidad" icon={<Eye size={16} />}>
        <BigValue
          ...
          unit="km"
          sub={w.current.visibility >= 10000 ? "Excelente" : "Reducida"}
        />
      </WidgetShell>
    ),
  },
  ...
  },
  {
    ...
    render: (w) => (
      <WidgetShell title="Precipitación" icon={<CloudRain size={16} />}>
        <BigValue
          value={w.current.precipitation.toFixed(1)}
          unit="mm"
          sub={`Lluvia ${w.current.rain.toFixed(1)} mm`}
        />
      </WidgetShell>
    ),
  },
  ...
  },
  {
    ...
    render: (w) => (
      <WidgetShell title="Punto de rocío" icon={<Waves size={16} />}>
        <BigValue
          value={`${Math.round(w.current.dewPoint)}°`}
          sub={
            w.current.dewPoint >= 20
              ? "Ambiente bochornoso"
              : w.current.dewPoint >= 10
                ? "Ambiente agradable"
                : "Ambiente seco"
          }
        />
    ...
    render: (w) => {
      ...
      return (
        <WidgetShell
          ...
          accent="hsl(38 92% 60%)"
          footer={`${formatDuration(d.daylightSeconds)} de luz`}
        >
    ...
    render: (w) => {
        ...
        return (
          <WidgetShell title="Calidad del aire" icon={<Leaf size={16} />} accent="hsl(142 71% 45%)">
            <p className="text-sm text-muted-foreground">
              Sin datos disponibles para esta ubicación.
            </p>
          </WidgetShell>
        );
      }
      ...
      return (
        <WidgetShell
          ...
          icon={<Leaf size={16} />}
          accent={band.color}
          footer={`PM2.5 ${Math.round(w.airQuality.pm25)} · PM10 ${Math.round(w.airQuality.pm10)} µg/m³`}
        >
